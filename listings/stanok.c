#include "sys/sys.h"
#include "include/platform/router/stanok.h"
#include "include/platform/router/stanok_desc.h"
#include "include/platform/router/operator_pult.h"
#include "include/func/axis.h"
#include "include/func/spnd.h"
#include "include/cnc/errors.h"
#include "include/cnc/mt.h"
#include "include/func/misc.h"
#include "include/cnc/cnc.h"
#include "include/func/cs.h"
#include "include/platform/router/tools.h"
...
AxisJogSettings axisJogSettings[MACHINE_NUM_AXES] = {
    {.speedSelect = {0.1, 1, 10, 30}, .speedRapid = 40, .speedRapidAuto = 50,
     .incSpeed = 30, .incStep = {2., 20., 200., 2000.}},
    {.speedSelect = {0.1, 1, 10, 30}, .speedRapid = 40, .speedRapidAuto = 50,
     .incSpeed = 30, .incStep = {2., 20., 200., 2000.}},
    {.speedSelect = {0.1, 1, 10, 30}, .speedRapid = 40, .speedRapidAuto = 50,
     .incSpeed = 30, .incStep = {2., 20., 200., 2000.}},
    {.speedSelect = {0.1, 1, 10, 30}, .speedRapid = 40, .speedRapidAuto = 50,
     .incSpeed = 30, .incStep = {2./360., 20./360., 200./360., 2000./360.}},
    {.speedSelect = {0.1, 1, 10, 30}, .speedRapid = 40, .speedRapidAuto = 50,
     .incSpeed = 30, .incStep = {2./360., 20./360., 200./360., 2000./360.}},
};
USER_SAVE(axisJogSettings);

MachineSettings machineSettings = MACHINE_SETTINGS;
USER_SAVE(machineSettings);
...
void initMachine() {
    ...
    mt.ncStopReq = 0;
    mt.ncFollowUpReq = 0;
    CNC.channel[MACHINE_CH1].runtime.T = 0;

    Coord[MACHINE_CH1_CS].LHSize = 1024;
    Coord[MACHINE_CH1_CS].LHDistance = 1024;
    Coord[MACHINE_CH1_CS].TPSize = 100;
    Coord[MACHINE_CH1_CS].CCSize = 256;
    Coord[MACHINE_CH1_CS].CCDistance = 32;
    Coord[MACHINE_CH1_CS].FeedHoldSlew = 0.0001;
    Coord[MACHINE_CH1_CS].TimeBaseSlew = 0.0001;
    Coord[MACHINE_SPND1_CS].FeedHoldSlew = 0.0001;
    Coord[MACHINE_SPND1_CS].TimeBaseSlew = 0.0001;
    initChannel(MACHINE_CH1, MACHINE_CH1_CS, CNC.channel[MACHINE_CH1].runtime);
...
//    case mtError: {
//        // Обработка нажатия кнопки Reset
//        if (mtDesc.PultIn.modeReset) {
//            processReset();
//            timerStart(mtDesc.timerReset,
//                       MT_TIME_RESET);
//            mtDesc.State = mtReseting;
//        }
//        break;
//    }
//    case mtReseting:{
//        if (timerTimeout(mtDesc.timerReset)) {
//            for (int i=0; i<MACHINE_NUM_AXES; i++) {
//                axisForceKill(i);
//            }
//            if (!mtDesc.PultIn.modeReset) {
//                mtDesc.State=mtNotReady;
//            }
//        }
//        break;
//    }
...
void mtUpdateCNCIndication() {
    ...
    case cncAuto:
        ...
        //mt.PultOut.modeVirtual = CNC.modeAutoVirtual;
    ...
    // посдсветка кнопки разрешения работы переносного пульта
//    if (CNC.enablePortablePult) {
//        mt.PultOut.portablePultEna = 1;
//    } else {
//        mt.PultOut.portablePultEna = 0;
//    }

    // индикация активной оси
    //mt.PultOut.xPlus    = axesControl.activeAxis == axisX;
    //mt.PultOut.rapid    = axesControl.activeAxis == axisY;
    //mt.PultOut.xMinus   = axesControl.activeAxis == axisZ;
...
/*
    if (Servo[0].IO[3].Status & 1) {
        mt.PortablePultIn.PultBtn[0] = Servo[0].IO[3].DataIn[0];
        mt.PortablePultIn.PultBtn[1] = Servo[0].IO[3].DataIn[1];
...
void mtControlRequest()
{
    ...
    // Корректоры S и F
    // TODO: параметризировать/реализовать в виде команд
